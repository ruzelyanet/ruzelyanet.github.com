...
/*#__PURE__*/
function () {
  function erasableLine() {
    _classCallCheck(this, erasableLine);
  ...
  _createClass(erasableLine, [{
    key: "_styles",
    value: function _styles() {
      this.select.style.whiteSpace = 'nowrap';

      for (var i = 0; i < this.items.length; i++) {
        this.items[i].style.display = 'none';
        var children = this.items[i].children;

        for (var c = 0; c < children.length; c++) {
          children[c].style.cssText = 'width: 0%; overflow: hidden;';
        }
      }

      this.items[0].style.display = 'block';
      var currentChildren = this.items[0].children;

      for (var _c = 0; _c < currentChildren.length; _c++) {
        currentChildren[_c].style.cssText = 'width: 100%;';
      }
    }
  }, {
    key: "_lineEffect",
    value: function _lineEffect(item, interval) {
      return new Promise(function (resolve, reject) {
        var children = item.children;
        var maxEffectDuration = 300;
        var devideInterval = interval / 3;
        var setIntervalBetweenLine = devideInterval / children.length;
        var standingPeriod = interval - (devideInterval + setIntervalBetweenLine);
        var checkIntervalLines = setIntervalBetweenLine <= maxEffectDuration ? setIntervalBetweenLine : maxEffectDuration;
        var commonEffectTimeLine = checkIntervalLines * children.length; //-- Open lines

        setTimeout(function () {
          var i = 0;
          var openEffects = setInterval(function () {
            children[i].style.cssText = 'width: 100%;';
          ...
          function stopOpenEffects() {
            clearInterval(openEffects);
          }
        }, 100); //-- Close lines

        setTimeout(function () {
          var i = 0;
          var closeEffects = setInterval(function () {
            children[children.length - (i + 1)].style.cssText = 'width: 0%;';

            if (i == children.length - 1) {
              setTimeout(function () {
                stopCloseEffects();
              }, commonEffectTimeLine);
            } else {
              i++;
            }
          }, checkIntervalLines);

          function stopCloseEffects() {
            clearInterval(closeEffects);
            resolve('next');
          }
        }, standingPeriod);
      });
    ...
    key: "_next",
    value: function _next(i, callback) {
      this._screenSlider(i);

      callback.call(this);
    }
  }, {
    key: "_change",
    value: function _change(i) {
      return i;
    }
  }, {
    key: "_screenSlider",
    value: function _screenSlider(visibleItem) {
      var _this = this;
      ...
      this.items[visibleItem].style.display = 'block'; //====

      this._lineEffect(this.items[visibleItem], this.interval).then(function (val) {
        if (visibleItem == _this.items.length - 1) {
          visibleItem = 0;
        } else {
          visibleItem++;
        }

        _this._next(visibleItem, function () {
          this.change(visibleItem);
        });
      });
    }
  }, {
    key: "_switch",
    value: function _switch(num) {
      var i = 0;
      var $this = this;
    ...
    key: "_clg",
    value: function _clg(i) {
      console.log(i);
    }
  }, {
    key: "launch",
    value: function launch(options, callback) {
      //--options
      ...
      this.items = this.select.children;
      ...
      this.itemsArray[0].active = true; //=== Set firs styles

      this._styles();

      if (this.items.length > 1) {
        //=== first animate lines effect
        this._lineEffect(this.items[0], this.interval); //=== Start Switcher


        this._switch(this.items.length - 1);
      }

      callback.call(this, 111, 555, 888);
    }
  }]);
